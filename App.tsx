...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadCloudConfig = async () => {
      try {
        ...
        console.warn("[App] Failed to load config from cloud, using local defaults", e);
      }
    };
    loadCloudConfig();
  }, []);
  ...
  useEffect(() => {
    const updateTitle = () => {
      const cfg = getConfig();
      const name = cfg.appName || 'Paydone.id';
      const desc = cfg.appDescription || 'Financial Cockpit';
      document.title = `${name} | ${desc}`;
    };
  ...
  const loadLocalDataIntoState = React.useCallback((userId: string) => {
  ...
  const performBackgroundSync = React.useCallback(async (userId: string) => {
    try {
      const config = getConfig();
  ...
  }, [currentUserId, userRole, loadLocalDataIntoState]);
  ...
  const handleManualSync = React.useCallback(async () => {
      ...
      const fullPayload = {
          ...
          // V50.00 Additions
          qaScenarios: globalDB.qaScenarios || [],
          config: globalDB.config || {},
          banks: globalDB.banks || [],
          baConfigurations: globalDB.baConfigurations || []
      };
