...
import { 
  ...
  ArrowRight, ArrowLeft, AlertTriangle, Cloud, Monitor, MessageSquare, Bot, User, Send, BrainCircuit, Clipboard, PlayCircle, PauseCircle, RotateCcw, Crown, Paste,
...
export default function ServerCompare() {
  ...
  const fetchRemote = async () => {
    ...
    try {
      ...
      setRemoteScript(''); // Keep empty so diff logic works (100% diff)
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex flex-col space-y-4 h-[calc(100vh-100px)]">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm shrink-0">
          ...
          <div className="flex gap-3">
              <div className={`px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2 border ${diffCount === 0 ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                  {diffCount === 0 ? <CheckCircle2 size={14}/> : <AlertTriangle size={14}/>}
                  {diffCount === 0 ? 'Synced' : `${diffCount} Discrepancies`}
              </div>
              ...
              {chatHistory.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-70">
                      ...
                      <button 
                        ...
                        disabled={isThinking}
                        className="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition flex items-center gap-2"
                      >
