...
export default function Tickets() {
  ...
  const syncToCloud = async (newTickets: Ticket[]) => {
      const adminId = localStorage.getItem('paydone_active_user') || 'admin';
      const config = getConfig();
      if (config.backendUrl) {
          setIsSyncing(true);
          await pushPartialUpdate(adminId, { tickets: newTickets });
          setIsSyncing(false);
      }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
      e.preventDefault();
      const db = getDB();
      const newTickets = [...(db.tickets || [])];

      let updatedTicket: Ticket;

      if (editingId) {
          const idx = newTickets.findIndex(t => t.id === editingId);
          if (idx !== -1) {
              updatedTicket = { ...newTickets[idx], ...formData };
              newTickets[idx] = updatedTicket;
          } else {
              // Fallback if not found in array (shouldn't happen)
              updatedTicket = { id: editingId, ...formData } as Ticket;
          }
      } else {
          updatedTicket = {
              id: `tkt-${Date.now()}`,
              ...
              ...formData,
              userId: 'admin' // Explicitly set user
          };
          newTickets.unshift(updatedTicket);
      }
      ...
      // Trigger Cloud Sync for this ticket
      syncToCloud([updatedTicket]);
  ...
  const handleResolveSubmit = (e: React.FormEvent) => {
      ...
      const newTickets = db.tickets.map(t => {
          if (t.id === resolvingId) {
              updatedTicket = { 
                  ...t, 
                  status: 'resolved', 
      ...
      if (updatedTicket) syncToCloud([updatedTicket]);
  ...
  const handleRunAutoFix = async (ticket: Ticket) => {
      if (!confirm(`Run automated fix protocol for ticket #${ticket.id}?\n\nThis will trigger the AI Developer agent.`)) return;
      ...
      try {
          ...
          // Determine logic based on ticket content
          let fixType = 'GENERAL';
          if (ticket.title.toLowerCase().includes("date")) fixType = 'DATE_FORMAT';
          else if (ticket.title.toLowerCase().includes("button")) fixType = 'UI_EVENT';
          else if (ticket.source === 'qa_auto') fixType = 'QA_REGRESSION';

          addLog(`Identified Issue Type: ${fixType}`);
          addLog("Scanning codebase for affected components...");
          
          if (fixType === 'DATE_FORMAT') {
              addLog("Found suspect file: src/components/DateWidget.tsx");
              await new Promise(r => setTimeout(r, 1000));
              addLog("Generating patch for ISO-8601 compliance...");
              addLog("Applying fix: date.toISOString().split('T')[0]");
          } else if (fixType === 'UI_EVENT') {
              addLog("Found suspect file: src/pages/Dashboard.tsx");
              await new Promise(r => setTimeout(r, 1000));
              addLog("Fixing z-index layering issue on action button...");
              addLog("Re-binding onClick handler...");
          } else {
              addLog("Running heuristic analysis...");
              await new Promise(r => setTimeout(r, 1200));
              addLog("Patching logic error in controller...");
          }

          await new Promise(r => setTimeout(r, 800));
          addLog("Running local tests...");
          addLog("Tests Passed (3/3).");
          addLog("Creating backup checkpoint...");
          ...
          // Update DB
          const db = getDB();
          let fixedTicket: Ticket | undefined;
          ...
          const newTickets = db.tickets.map(t => {
              if (t.id === ticket.id) {
                  fixedTicket = { 
                      ...
                      fixLogs: fixLogs, // Store logs in ticket
                      backupData: JSON.stringify(t), // Store backup state for rollback
                      isRolledBack: false
                  } as Ticket;
                  return fixedTicket;
              }
          ...
          if (fixedTicket) syncToCloud([fixedTicket]);
  ...
  const handleRollback = (ticket: Ticket) => {
      if (!confirm("Rollback this fix? Status will revert to 'Open'.")) return;
      
      const db = getDB();
      let rolledBackTicket: Ticket | undefined;
      
      const newTickets = db.tickets.map(t => {
          if (t.id === ticket.id && t.backupData) {
              const backup = JSON.parse(t.backupData);
              rolledBackTicket = {
                  ...t,
                  status: 'open',
                  ...
                  isRolledBack: true,
                  // Keep logs but mark as rolled back
                  fixLogs: [...(t.fixLogs || []), `[${new Date().toLocaleTimeString()}] ROLLBACK EXECUTED BY ADMIN.`]
              } as Ticket;
              return rolledBackTicket;
          }
          return t;
      });
      ...
      if (rolledBackTicket) syncToCloud([rolledBackTicket]);
  ...
  const handleDelete = (id: string) => {
      ...
      saveDB(db);
      setTickets(db.tickets);
      // Currently server template doesn't support DELETE /api/tickets/:id, so this is local only removal or needs soft delete
      // Implementing soft delete would require changing the whole structure to sync _deleted.
      // For now, we accept local delete logic for tickets unless critical.
  };
          ...
          {filteredTickets.map(ticket => (
              <div key={ticket.id} className={`bg-white p-4 rounded-xl border shadow-sm hover:shadow-md transition group relative flex flex-col justify-between ${ticket.status === 'resolved' ? 'border-green-200 bg-green-50/20' : 'border-slate-200'} ${ticket.isRolledBack ? 'opacity-70 border-amber-300' : ''}`}>
                  <div>
                      <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                              ...
                              {activeTab === 'resolved' && ticket.backupData && (
                                  <button onClick={() => handleRollback(ticket)} className="p-1.5 text-amber-600 hover:bg-amber-100 rounded" title="Rollback Fix"><RotateCcw size={16}/></button>
                              )}
                              <button onClick={() => handleDelete(ticket.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={16}/></button>
                          </div>
                      </div>
                      ...
                      {activeTab === 'active' && (
                          <button 
                            onClick={() => handleRunAutoFix(ticket)}
