...
export default function Planning({ tasks, debts, allocations, onToggleTask, onToggleAllocation }: PlanningProps) {
  ...
  useEffect(() => {
    ...
    // Debt Tasks Generator
    const debtTasks: TaskItem[] = debts.map(debt => {
       const rawDay = typeof debt.dueDate === 'number' ? debt.dueDate : 1;
       const dueDateDay = Math.max(1, Math.min(31, rawDay)); 
       
       let targetDate = new Date(currentYear, currentMonth, dueDateDay);
       if (isNaN(targetDate.getTime())) targetDate = new Date();
       if (today.getDate() > dueDateDay) targetDate = new Date(nextMonthYear, nextMonth, dueDateDay);
       
       const isoDate = targetDate.toISOString().split('T')[0];
       const db = getDB();
       // Check if installment exists and is paid for this month
       const isPaid = db.debtInstallments?.some(inst => 
           inst.debtId === debt.id && 
           inst.status === 'paid' && 
           new Date(inst.dueDate).getMonth() === targetDate.getMonth() &&
           new Date(inst.dueDate).getFullYear() === targetDate.getFullYear()
       );
  ...
  }, [tasks, debts, allocations, processingId]); 
  ...
  const handleToggle = async (task: TaskItem) => {
      ...
      // DEBT TASK Logic
      if (task.id.startsWith('auto-pay-task-')) {
          const datePart = task.id.slice(-10); 
          const debtId = task.id.replace('auto-pay-task-', '').replace(`-${datePart}`, '');
          const db = getDB();
          const targetDate = new Date(datePart);
          
          const installment = db.debtInstallments?.find(i => 
              i.debtId === debtId && 
              new Date(i.dueDate).getMonth() === targetDate.getMonth() &&
              new Date(i.dueDate).getFullYear() === targetDate.getFullYear()
          );
          ...
          if (installment) {
              ...
              // Update local state is tricky here because Planning reads from derived props mostly
              // We rely on parent refresh or wait for sync. 
              // To make UI snappy, we update local DB directly here as Planning is a consumer.
              
              // Update local DB array
              if (db.debtInstallments) {
                  const idx = db.debtInstallments.findIndex(i => i.id === installment.id);
                  if (idx !== -1) db.debtInstallments[idx] = updatedInstallment;
                  saveDB(db);
              }
              
              await pushPartialUpdate(task.userId, { debtInstallments: [updatedInstallment] });
          }
      } 
      // ALLOCATION TASK Logic
      else if (task.context === 'Allocation' && onToggleAllocation) {
      ...
      // MANUAL TASK Logic
          ...
          if (userId) {
              ...
              const updatedTasks: TaskItem[] = userData.tasks.map(t => {
                  if (t.id === task.id) {
              ...
              saveUserData(userId, { ...userData, tasks: updatedTasks });
              await pushPartialUpdate(userId, { tasks: updatedTasks.filter(t => t.id === task.id) });
          }
      }
